...
/**
 * Unit tests for {@link TftpPacket} (client-side copy). Mirrors the server
 * module's tests because RFC 1350 requires identical wire format on both
 * ends — the two copies must evolve in lock-step.
 */
class TftpPacketTest {
    ...
    void buildError_encodesCodeAndNullTerminatedMessage() {
        ...
        assertEquals(5, err[1]);
        assertEquals(0, err[2]);
        assertEquals(1, err[3]);
